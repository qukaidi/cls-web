//可信任的私有仓库地址
   ...
   def APP_NAME = "cls-web"
   def PROJECT_URL="http://127.0.0.1:8082/"
   ...
   pipeline{ 
    ...
    //  git branch: "master", credentialsId: '1', url: "http://127.0.0.1/pcc-check-list/cls-web.git"
     //     }
            ...
            steps {
                ws("${env.WORKSPACE}@2") {
                    sh 'ls -l'
                    sh "docker build --build-arg DIST_PATH=${currentBuild.absoluteUrl} -t ${REGISTRY_URL}/${APP_NAME}:${APP_TAG} -f Dockerfile_${BRANCH_NAME} . "
                }
    ...
    // }
        stage('run docker images for test') {
            when{
                branch 'test'
            }
            steps {
                script {
                      try {
                          ...
                          sh """docker -H 127.0.0.1:2375 rm -f cls-web-test"""
                      } catch(Exception e) {
                          echo 'Handle the exception!'
                      }
                }
                sh """docker -H 127.0.0.1:2375 pull ${REGISTRY_URL}/${APP_NAME}:${APP_TAG}
                docker -H 127.0.0.1:2375 run -d --network=cls -p 8082:80 --name=cls-web-test ${REGISTRY_URL}/${APP_NAME}:${APP_TAG}
                """

            }
        }
               stage('run docker images for atj prod') {
                   when{
                       branch 'atj-master'
                   }
                   steps {
                       script {
                           try {
                               echo 'stop && remove old container'
                               sh """docker -H 127.0.0.1:2375 rm -f cls-web-atj"""
                           } catch(Exception e) {
                               echo 'Handle the exception!'
                           }
                       }
                       sh """docker -H 127.0.0.1:2375 pull ${REGISTRY_URL}/${APP_NAME}:${APP_TAG}
                docker -H 127.0.0.1:2375 run -d --network=cls -p 82:80 --name=cls-web-atj ${REGISTRY_URL}/${APP_NAME}:${APP_TAG}
                """

                   }
               }
               stage('run docker images for atd prod') {
                   when{
                       branch 'atd-master'
                   }
                   steps {
                       script {
                           try {
                               echo 'stop && remove old container'
                               sh """docker -H 127.0.0.1:2375 rm -f cls-web-atd"""
                           } catch(Exception e) {
                               echo 'Handle the exception!'
                           }
                       }
                       sh """docker -H 127.0.0.1:2375 pull ${REGISTRY_URL}/${APP_NAME}:${APP_TAG}
                docker -H 127.0.0.1:2375 run -d --network=cls -p 10082:80 --name=cls-web-atd ${REGISTRY_URL}/${APP_NAME}:${APP_TAG}
                """
